...
print("\n" + "=" * 55)
print("XGBoost 3-class — RandomizedSearchCV Results")
print("=" * 55)
...
ax.set_title("XGBoost 3-class — Confusion Matrix", fontsize=13, fontweight='bold')
plt.tight_layout()
plt.savefig(f"{PLOTS_DIR}/cm_xgb_3class.png", dpi=150)
plt.close()
print(f"\nConfusion matrix saved: cm_xgb_3class.png")

# ── Save model ───────────────────────────────────────────────
with open(f"{MODELS_DIR}/model_xgb_3class.pkl", 'wb') as f:
    pickle.dump(best, f)
print(f"Model saved          : model_xgb_3class.pkl")
